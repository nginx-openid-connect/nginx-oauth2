# How To Locally Test Containerized NGINX Plus OIDC for Keycloak Integration

Take the following steps to run NGINX Plus in a Docker container and test OIDC for Keycloak integration.
...
- **Run** a Web Browser with `http://nginx.keycloak.test:11000`, and enter an `IdP application`'s `client ID`:

  ![](./img/run-browser.png)

- **Login** to click `Sign in` button:

  Enter your name and password which are registered in Keycloak.
